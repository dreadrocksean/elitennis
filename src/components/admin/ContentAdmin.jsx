          ...
          {bio.paragraphs.map((p, i) => (
            <Field key={i} label={`Paragraph ${i + 1}`}>
              <textarea
                ...
                value={p}
                onChange={(e) => setBioPara(i, e.target.value)}
              />
            </Field>
          ))}
        </div>
      ...
      <Panel title="Stats strip" description="Four small credibility stats next to your photo.">
        <div className="grid gap-3 sm:grid-cols-4">
          {stats.map((s, i) => (
            <div key={i} className="rounded-2xl border border-forest/10 p-3">
              <input
                className="input mb-2 text-center font-display"
                value={s.value}
              ...
              <div key={i} className="flex gap-2">
                <input
                  ...
                  onChange={(e) =>
                    setPricing({
                      ...pricing,
                      perks: pricing.perks.map((p, idx) => (idx === i ? e.target.value : p)),
                    })
                  }
                />
                <button
                  onClick={() =>
                    setPricing({ ...pricing, perks: pricing.perks.filter((_, idx) => idx !== i) })
                  }
