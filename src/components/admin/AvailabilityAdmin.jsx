                    ...
                    <button
                      ...
                      className={[
                        ...
                        on
                          ...
                          : 'bg-forest-50 text-forest-700/70 hover:bg-forest-100',
                      ].join(' ')}
                    >
                      {formatTime(t)}
                    </button>
      ...
      <Panel title="Booking rules & blackout dates">
        <div className="grid gap-6 sm:grid-cols-2">
          <Field
            ...
            hint="How far ahead a client must book. e.g. 12 = no same-evening bookings."
          >
            <input
              type="number"
              min="0"
          ...
          <div>
            ...
            <div className="mt-3 flex flex-wrap gap-2">
              {blackouts.length === 0 && (
                <span className="text-xs text-forest-700/50">No blackout dates.</span>
              )}
              {blackouts.map((d) => (
                <span
                  key={d}
                  className="inline-flex items-center gap-1.5 rounded-full bg-forest-50 px-3 py-1.5 text-xs font-medium text-forest"
                >
                  {d}
                  <button
